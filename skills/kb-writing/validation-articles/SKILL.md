...
| Pass/fail pattern description line | Each code example must be preceded by exactly one short sentence that describes what the example shows. It must contain a verb, use present tense, and describe the example — not explain why it passes or fails. Keep it concise: no subordinate clauses, no added reasoning. Good: "A list contains the required `listitem` children." Bad: "A list with the required `listitem` children" (no verb); "A menu contains a list for its visual display, which causes a semantics conflict" (too long, explains the failure) |
