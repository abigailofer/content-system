...
| Pass/fail pattern structure | Each code example must be preceded by a title that describes what the example shows. The title must contain a verb, use present tense, and describe the example rather than explain why it passes or fails (good: "A list contains the required `listitem` children"; bad: "A list with the required `listitem` children" — no verb; "A menu contains a list, which causes a semantics conflict" — explains the failure). The title format depends on whether an explanation sentence follows: (1) if an explanation sentence is present, the title must be an H3 with no trailing period; (2) if no explanation sentence is present, the title must be body text, not a heading. Explanation sentences must be used consistently — if any pattern in the section has one, all patterns in that section must have one |
